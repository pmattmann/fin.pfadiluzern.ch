...
class SpesenMelden extends Form
{
    public function __construct($name = null, $konten = [], array $options = [])
    {
        ...
        $spesen = new Element\Collection('spesen');
        $spesen->setLabel('Spesen');
        $spesen->setCount(1);
        $spesen->setShouldCreateTemplate(true);
        $spesen->setTemplatePlaceholder('__placeholder__');
        $spesen->setTargetElement(new SpesenFieldset('spesenItem', $konten));
